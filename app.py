...
# --- CODE EMAS (LOGIC PRESERVED + EXCLUDED LIST ADDED) ---
def run_strict_procedure(file, q_min, area_min):
    ...
    # Track items removed by Quality and Area filter if needed, 
    # but based on your request, we focus on "excluded compounds" (Artifacts)
    ...
    # Capture the excluded ones before they are dropped
        ...
        try:
            ...
            with t4:
                ...
                if not rt_issues.empty:
                    st.info(f"Found **{len(rt_issues)}** compounds with significant RT shifts. Retained.")
                    st.table(rt_issues[['Hit Name', 'RT (min)', 'RT_Diff']])
                else: st.success("No significant RT shifts detected.")
            with t5:
                st.info("Compounds excluded based on blacklist (e.g., siloxanes, phthalates)")
                st.dataframe(ex_s)
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                ws_dash.set_column('B:B', 30); ws_dash.set_column('C:C', 30)
                ...
                # --- SHEET 3: EXCLUDED COMPOUNDS (NEW!) ---
                ex_sheet = 'Excluded_Artifacts'
                h_s_ex = h_s.copy()
                h_s_ex.iloc[0,0] = f"{h_s_ex.iloc[0,0]} (Excluded Artifacts ❌)"
                h_s_ex.to_excel(writer, sheet_name=ex_sheet, startrow=2, index=False, header=False)
                ex_s.to_excel(writer, sheet_name=ex_sheet, startrow=11, index=False, header=True)
                
                # Apply conditional formatting to report sheet (preserving your original features)
