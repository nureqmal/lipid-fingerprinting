...
# --- CODE EMAS (LOGIC PRESERVED + EXCLUDED ADDED) ---
def run_strict_procedure(file, q_min, area_min):
    ...
    # Capture artifacts (e.g. Octasiloxane) before they are filtered out
        ...
        try:
            ...
            with t1:
                def highlight_blank(row):
                    ...
                    return styles
                st.dataframe(df_b.style.apply(highlight_blank, axis=1), use_container_width=True)
            ...
            with t2:
                def highlight_sample(row):
                    ...
                    if row['In_Blank'] == "YES": styles = ['background-color: #FFEB9C' for _ in row.index]
                        ...
                        styles[rt_idx] = 'background-color: #002060; color: white'
                    return styles
                st.dataframe(df_s.style.apply(highlight_sample, axis=1), use_container_width=True)
            
            with t3: st.dataframe(df_final.drop(columns=['In_Blank', 'RT_Diff']), use_container_width=True)
            ...
            with t5:
                st.info(f"Captured {len(ex_s)} Blacklisted Artifacts (Octasiloxane, etc.)")
                st.dataframe(ex_s, use_container_width=True)
...
with tab2:
    ...
    if m_blank and m_samples:
        try:
            ...
            st.subheader("Ready for PCA Table")
            st.dataframe(df_pca)
