...
if sample_file and blank_file:
    try:
        ...
        def apply_contaminant_pink(row, styles):
            if row.get('Chemical_Status') == "Review (Potential Contaminant)" and 'Hit Name' in row.index:
                hit_idx = row.index.get_loc('Hit Name')
                styles[hit_idx] = 'background-color: #FFC0CB'
            return styles
        ...
        with t1:
            def highlight_blank(row):
                ...
                return apply_contaminant_pink(row, styles)
        ...
        with t3:
            ...
            df_final_display = df_final.drop(columns=['In_Blank', 'RT_Diff'])
            st.dataframe(df_final_display.style.apply(highlight_final, axis=1))
        ...
        # --- EXPORT LOGIC ---
        custom_filename = st.text_input("📁 Rename your file before download", value="eg. SF_Hexane-1")
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            ws_dash.merge_range('B2:E2', 'ANALYTICAL SUMMARY', header_fmt)
            ...
            ws_dash.write('C11', 'Matched in Blank/Sample (Shared Compound)')
            ws_dash.write('B12', 'Navy Blue RT Cell', navy_fmt)
            ws_dash.write('C12', 'RT Shift Detected (Retained)')
            ws_dash.write('B13', 'Pink Compound Cell', pink_fmt)
            ws_dash.write('C13', 'Review (Potential Contaminant)')
            ...
            # Final corrected unique section
            s3 = s2 + len(df_s) + 15
            fh = h_s.copy()
            fh.iloc[0, 0] = f"{fh.iloc[0, 0]} (CORRECTED UNIQUE)"
            fh.to_excel(writer, sheet_name=rs, startrow=s3+1, index=False, header=False)

            df_final_export = df_final.drop(columns=['In_Blank', 'RT_Diff'])
            df_final_export.to_excel(writer, sheet_name=rs, startrow=s3+10, index=False, header=False)

            ws_rep = writer.sheets[rs]

            # Blank table formatting
            b_rt_idx = df_b.columns.get_loc('RT (min)')
            b_match_idx = df_b.columns.get_loc('In_Sample')
            b_hit_idx = df_b.columns.get_loc('Hit Name')
            b_status_idx = df_b.columns.get_loc('Chemical_Status')
            b_match_col = xl_col_to_name(b_match_idx)
            b_status_col = xl_col_to_name(b_status_idx)

            ws_rep.conditional_format(
                11, 0, 11+len(df_b), len(df_b.columns)-1,
                {'type': 'formula', 'criteria': f'=${b_match_col}12="YES"', 'format': yellow_fmt}
            )
            ws_rep.conditional_format(
                11, b_rt_idx, 11+len(df_b), b_rt_idx,
                {'type': 'formula', 'criteria': f'=${b_match_col}12="RT_SHIFT_DETECTED"', 'format': navy_fmt}
            )
            ws_rep.conditional_format(
                11, b_hit_idx, 11+len(df_b), b_hit_idx,
                {'type': 'formula', 'criteria': f'=${b_status_col}12="Review (Potential Contaminant)"', 'format': pink_fmt}
            )

            # Sample table formatting
            s_rt_idx = df_s.columns.get_loc('RT (min)')
            s_match_idx = df_s.columns.get_loc('In_Blank')
            s_hit_idx = df_s.columns.get_loc('Hit Name')
            s_status_idx = df_s.columns.get_loc('Chemical_Status')
            s_match_col = xl_col_to_name(s_match_idx)
            s_status_col = xl_col_to_name(s_status_idx)
            sample_first_excel_row = s2 + 11

            ws_rep.conditional_format(
                s2+10, 0, s2+10+len(df_s), len(df_s.columns)-1,
                {'type': 'formula', 'criteria': f'=${s_match_col}{sample_first_excel_row}="YES"', 'format': yellow_fmt}
            )
            ws_rep.conditional_format(
                s2+10, s_rt_idx, s2+10+len(df_s), s_rt_idx,
                {'type': 'formula', 'criteria': f'=${s_match_col}{sample_first_excel_row}="RT_SHIFT_DETECTED"', 'format': navy_fmt}
            )
            ws_rep.conditional_format(
                s2+10, s_hit_idx, s2+10+len(df_s), s_hit_idx,
                {'type': 'formula', 'criteria': f'=${s_status_col}{sample_first_excel_row}="Review (Potential Contaminant)"', 'format': pink_fmt}
            )

            # Final Corrected Unique table formatting
            f_hit_idx = df_final_export.columns.get_loc('Hit Name')
            f_status_idx = df_final_export.columns.get_loc('Chemical_Status')
            f_status_col = xl_col_to_name(f_status_idx)
            final_first_excel_row = s3 + 11

            ws_rep.conditional_format(
                s3+10, f_hit_idx, s3+10+len(df_final_export), f_hit_idx,
                {'type': 'formula', 'criteria': f'=${f_status_col}{final_first_excel_row}="Review (Potential Contaminant)"', 'format': pink_fmt}
            )
